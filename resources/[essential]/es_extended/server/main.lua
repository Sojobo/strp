	...
	elseif type == 'item_money' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > playerCash or playerCash < 1) then
				...
				local pickupLabel = ('~b~%s~s~ [~g~%s~s~]'):format(_U('cash'), _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
	...
	elseif type == 'item_account' then
		...
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				local pickupLabel = ('~b~%s~s~ [~g~%s~s~]'):format(account.label, _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
...
AddEventHandler('esx:onPickup', function(id)
	...
	if pickup.type == 'item_standard' then
		...
		local canTake   = (xInventorylimit - xInventoryWeight) or 0
		...
		if remaining > 0 then
			...
			local pickupLabel = ('~b~%s~s~ [~b~%s~s~]'):format(item.label, remaining)
...
ESX.RegisterServerCallback("inventory:getInfo", function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
	local xInventoryWeight = xPlayer.getInventoryWeight()
	local xInventorylimit = xPlayer.getInventoryWeightMax()
	cb(xInventoryWeight, xInventorylimit)
end)
