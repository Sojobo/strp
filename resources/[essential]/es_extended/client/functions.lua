...
ESX.ShowAdvancedNotificationForJob = function(job, subject, msg, icon, iconStyle)
	if ESX.GetPlayerData().job.name == job then
		ESX.ShowAdvancedNotification(subject, msg, icon, iconStyle)
	end
end

ESX.ShowNotification = function(msg)
	ESX.ShowAdvancedNotification("", msg, "fas fa-info", "blue")
end

ESX.ShowAdvancedNotification = function(subject, msg, icon, iconStyle)
	msg = string.gsub(msg, "~[a-z]~", "")
	TriggerEvent("pNotify:SendNotification", {
		text = '<h1>' .. subject .. '</h1><p>' .. msg .. '</p>',
		timeout = 10000,
		queue = "right",
		layout = "centerRight",
		theme = "notification",
...
ESX.Game.SetVehicleProperties = function(vehicle, props)
	...
	TriggerEvent("ls:newVehicle", vehicle, tostring(GetVehicleNumberPlateText(vehicle)), true)
end
...
ESX.ShowInventory = function()
	...
	ESX.TriggerServerCallback('inventory:getInfo', function(xInventoryWeight, xInventorylimit)
		inventoryWeight = xInventoryWeight
		inventoryLimit = xInventorylimit
	end)

	while inventoryWeight == nil or inventoryLimit == nil do
		Citizen.Wait(10)
	end
	...
	{
		title    = _U('inventory') .. " " .. inventoryWeight .. "/" .. inventoryLimit,
		align    = 'bottom-right',
		elements = elements,
	}, function(data, menu)
		menu.close()
...
AddEventHandler('esx:showAdvancedNotification', function(subject, msg, icon, iconStyle)
	ESX.ShowAdvancedNotification(subject, msg, icon, iconStyle)
end)

RegisterNetEvent('esx:ShowAdvancedNotificationForJob')
AddEventHandler('esx:ShowAdvancedNotificationForJob', function(job, subject, msg, icon, iconStyle)
	ESX.ShowAdvancedNotificationForJob(job, subject, msg, icon, iconStyle)
end)
