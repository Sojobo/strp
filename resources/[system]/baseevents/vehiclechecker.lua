...
local bannedVehicles = {
    ...
    ['fireTruk'] = { hash = GetHashKey('fireTruk'), canDrive = nil, hasTrunkAccess = nil },
	...
	while true do
		...
		if not isInVehicle and not IsPlayerDead(PlayerId()) then
			if DoesEntityExist(GetVehiclePedIsTryingToEnter(ped)) and not isEnteringVehicle then
				...
				if not vehicleBanned and not driverIsNetworkPlayer(vehicle) then
					Citizen.Trace(GetPedInVehicleSeat(vehicle, -1))
					if (GetPedInVehicleSeat(vehicle, -1) > 0) then
						-- if not IsVehicleModel(vehicle, GetHashKey("taxi")) then
