...
function log(sourcePlayer, targetPlayer, message)
	MySQL.Async.execute("INSERT INTO logs_jail (sourcePlayer, targetPlayer, message) VALUES (@sourcePlayer, @targetPlayer, @message)", {
...
AddEventHandler("jnj:releaseFromJail", function(targetPedId)
  ...
  elseif isJailed(getPlayerID(targetPedId)) then
    ...
    TriggerServerEvent("esx_policejob:unrestrain", targetPedId)
  else
  ...
  AddEventHandler("jnj:startCourtCase", function(targetPedId)
    ...
    if #jurors < 3 then
      ...
      if votes >= votesNeeded then
        ...
        TriggerClientEvent("jnj:courtCaseStatus", targetPedId, false) -- Back to jail we go
        TriggerClientEvent("chatMessage", -1, "^1In the case of ^2" .. targetPedName .. " ^1vs. the people of ^2" .. JailConfig.stateName ..
