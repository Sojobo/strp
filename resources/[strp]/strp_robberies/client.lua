			...
			if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceForMarkerToShow and isRobbing==false and robbableSpot.beingRobbed==false)then
				...
				if(Vdist(x,y,z,robbableSpot.x,robbableSpot.y,robbableSpot.z)<distanceToInteractWithMarker)then
					...
					if(IsControlJustPressed(1, keyToInteractWithRobbery))then
						TriggerEvent("robberies:policeCheck")
					end
				else
					if(isRobbing)then
						StopRobbery()
...
AddEventHandler("robberies:StartRobbery", function(cops)
	...
	Citizen.CreateThread(function()
		if (cops>=spotBeingRobbed.copsNeeded) then
			...
			ESX.TriggerServerCallback("robberies:hasLockpick", function(hasLockpick)
				if hasLockpick == false then
					ESX.ShowNotification("You do not have a lockpick!")
					return
				else
					TriggerEvent("robberies:robberyStartedNotification", spotBeingRobbed.name)
					TaskStartScenarioInPlace(GetPlayerPed(), "WORLD_HUMAN_WELDING", 0, true)
					isRobbing=true
					...
					Citizen.CreateThread(function()
						while isRobbing do
							if(spotBeingRobbed.isSafe)then
								DisplayHelpText("You are currently cracking the safe ("..spotBeingRobbed.timeToRob-currentSecondCount.." seconds left) (E to End)")
							else
								DisplayHelpText("You are currently prying open the cash register ("..spotBeingRobbed.timeToRob-currentSecondCount.." seconds left) (E to End)")
							end
							Citizen.Wait(0)
						end
					end)
...
function StopRobbery()
	...
	robbableSpots[spotBeingRobbed.name]=spotBeingRobbed
	TriggerEvent("robberies:robberyOverNotification", spotBeingRobbed.name)
	TriggerServerEvent("robberies:syncSpots", robbableSpots)
	spotBeingRobbed=nil
...
function RobberyOver()
	DisplayHelpText("You have sucesffully robbed " .. spotBeingRobbed.name .. "!")
...
AddEventHandler("robberies:robberyOverNotification", function(name)
	local notification = {
		subject = "Robbery",
		msg = "The robbery suspect has been seen fleeing " .. name .. "!",
...
AddEventHandler("robberies:robberyStartedNotification", function(name)
	local notification = {
		subject = "Robbery",
		msg = "A silent alarm has been triggered at " .. name .. ".",
		icon = "fas fa-store-alt",
		iconStyle = "red"
	}
	TriggerServerEvent("esx_service:notifyAllInService", notification, "police")
...
RegisterNetEvent("robberies:cashNotify")
AddEventHandler("robberies:cashNotify", function(cashAmount)
	local notification = {
		subject = "Robbery",
		msg = "You grabbed " .. cashAmount .. " from the robbery!",
		icon = "fas fa-store-alt",
		iconStyle = "black"
	}
	ESX.ShowAdvancedNotification(notification)
end)