	...
	while true do
		...
		for name,spot in pairs(robbableSpots)do
			if(spot.isBank==false)then
				local aFourth = tonumber(spot.currentMoney/4)
...
AddEventHandler("robberies:robberyOver", function(name)
	TriggerEvent("es:getPlayerFromId", source, function(target)
		if(target)then
			...
			TriggerClientEvent("robberies:cashNotify", source, robbableSpots[name].currentMoney)
...
ESX.RegisterServerCallback("robberies:hasLockpick", function(source, cb)
