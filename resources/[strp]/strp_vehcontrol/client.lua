...
local polmav_hash = {"polmav", "predator"}
local heliSirens = {}
...
function IsPolMav(veh)
	for _, v in pairs(polmav_hash) do
...
function Heli.IsVehicleSirenOn(veh)
	if IsPolMav(veh) and not IsThisModelABoat(GetEntityModel(veh)) then
		return heliSirens[veh] or false
	end
	return IsVehicleSirenOn(veh)
end

function Heli.SetVehicleSiren(veh, bool)
	if IsPolMav(veh) and not IsThisModelABoat(GetEntityModel(veh)) then
		TriggerServerEvent("lvc_SetHeliSiren", bool)
		return bool
	end
	return SetVehicleSiren(veh, bool)
end
...
function TogIndicStateForVeh(veh, newstate)
	if DoesEntityExist(veh) and not IsEntityDead(veh) and not IsPolMav(veh) then
...
RegisterNetEvent("lvc_SetHeliSirenClient")
AddEventHandler("lvc_SetHeliSirenClient", function(sender, status)
	local player_s = GetPlayerFromServerId(sender)
	local ped_s = GetPlayerPed(player_s)
	if DoesEntityExist(ped_s) and not IsEntityDead(ped_s) then
		if IsPedInAnyVehicle(ped_s, false) then
			local veh = GetVehiclePedIsUsing(ped_s)
			heliSirens[veh] = status
		end
	end
end)
	...
	while true do
			...
			if IsPedInAnyVehicle(playerped, false) then
				...
				if GetPedInVehicleSeat(veh, -1) == playerped then
					...
					--- IS EMERG VEHICLE ---
					if GetVehicleClass(veh) == 18 or IsPolMav(veh) then
						...
						if not Heli.IsVehicleSirenOn(veh) and state_lxsiren[veh] > 0 then
							PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
							SetLxSirenStateForVeh(veh, 0)
							count_bcast_timer = delay_bcast_timer
						end
						if not Heli.IsVehicleSirenOn(veh) and state_pwrcall[veh] == true then
							PlaySoundFrontend(-1, "NAV_UP_DOWN", "HUD_FRONTEND_DEFAULT_SOUNDSET", 1)
						...
						if not IsPauseMenuActive() then
							...
							-- HORN
							if IsDisabledControlPressed(0, 86) and not IsPolMav(veh) then
								actv_horn = true
							else
								actv_horn = false
							end
					...
					--- IS ANY LAND VEHICLE ---
					if GetVehicleClass(veh) ~= 14 and GetVehicleClass(veh) ~= 15 and GetVehicleClass(veh) ~= 16 and GetVehicleClass(veh) ~= 21 or IsPolMav(veh) then
