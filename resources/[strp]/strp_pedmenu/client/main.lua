		...
		if not IsPedInAnyVehicle(GetPlayerPed(-1)) then
			...
			if distance ~= -1 and distance < 10.0 then
				if distance ~= -1 and distance <= 2.0 then
					if IsPedDeadOrDying(playerped) or exports.esx_policejob:IsHandcuffed() == false then
...
function Start(player, ped)
	...
	while checking do
        ...
        if not (exports.esx_policejob:IsHandcuffed() == false or IsPedDeadOrDying(ped) or distance < 7.5) then
			checking = false
		end
