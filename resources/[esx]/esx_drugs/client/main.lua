...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(coords, Config.CircleZones.DrugDealer.coords, true) < 1.5 then
			ESX.ShowHelpNotification(_U('drug_dealer_prompt'))
			if IsControlJustReleased(0, Keys['E']) then
				wasOpen = true
				OpenDealer("DrugDealerItems")
			end
		elseif GetDistanceBetweenCoords(coords, Config.CircleZones.WineDealer.coords, true) < 1.5 then
			ESX.ShowHelpNotification(_U('wine_dealer_prompt'))
			if IsControlJustReleased(0, Keys['E']) then
				wasOpen = true
				OpenDealer("WineDealerItems")
			end
		else
			if wasOpen then
				wasOpen = false
...
function OpenDealer(dealersItems)
	ESX.UI.Menu.CloseAll()
	local elements = {}

	for k, v in pairs(ESX.GetPlayerData().inventory) do
		local price = Config[dealersItems][v.name]
