	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
		if not table.empty(vehicles) then
			for _,v in pairs(vehicles) do
				...
				elseif (v.state)  then
					labelvehicle = vehicleName..': Stored'
				else
					labelvehicle = vehicleName..': Withdrawn'
				end	
				table.insert(elements, {label =labelvehicle , value = v})
				
			end
		else
			table.insert(elements, {label ="There are no vehicles in the garage" , value = nil})
		end
		ESX.UI.Menu.Open(
		...
		function(data, menu)
			local elem = {}
			table.insert(elem, {label ="Take out vehicle" , value = 'get_vehicle_out'})
			table.insert(elem, {label ="Rename vehicle" , value = 'rename_vehicle'})
				...
				function(data2, menu2)
					if data2.current.value == "get_vehicle_out" then
                        if (data.current.value.fourrieremecano) then
                            TriggerEvent('esx:showNotification', 'Your vehicle has been impounded')
					...
					elseif data2.current.value == "rename_vehicle" then
						AddTextEntry('FMMC_KEY_TIP8', "Name of the desired vehicle")
...
function StockVehicleMenu(KindOfVehicle)
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		if GetPedInVehicleSeat(vehicle, -1) == playerPed then
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'Your trailer is in the garage')
					else
			...
			TriggerEvent('esx:showNotification', 'You are not the driver')
		end
	else
		TriggerEvent('esx:showNotification', 'No vehicle detected')
	end
end
			...
			if GotTrailer then
				...
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'The trailer is in the pound')
					else
						TriggerEvent('esx:showNotification', 'You cannot store this trailer in the pound')
					end
				end,trailerProps)
			else
				local vehicleProps  = ESX.Game.GetVehicleProperties(vehicle)
				ESX.TriggerServerCallback('eden_garage:stockvmecano',function(valid)
					if(valid) then
						...
						TriggerEvent('esx:showNotification', 'You cannot store this vehicle in the pound')
					end
				end,vehicleProps)
			end
		else
...
function ReturnVehicleMenu(garage, KindOfVehicle)
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		if not table.empty(vehicles) then
			...
			table.insert(elements, {label ="No vehicles out" , value = nil})
		end
		...
		function(data, menu)
			if data.current.value == 'fourrieremecano' then
				ESX.ShowNotification("Check with the police or a mechanic to find out how to get your vehicle back.")
			elseif data.current.value ~= nil then
				...
				if not iscaronearth then
					...
					ESX.ShowNotification("This vehicle is already out?")
				end				
			end
		end,
		function(data, menu)
